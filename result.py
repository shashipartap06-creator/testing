...
WHITE     = "#FFFFFF"
SEP       = "#1E2E42"
LIGHT_BG  = "#2A3A4A"  # Lighter background for entry fields
...
class resultClass:
    def __init__(self, root):
        ...
        self.root.geometry("1400x750+30+30")
    ...
    def _build_layout(self):
        ...
        left_panel = Frame(body, bg=CARD_BG, width=400)
        left_panel.pack(side=LEFT, fill=Y, padx=(0, 15))
        left_panel.pack_propagate(False)
        
        Label(left_panel, text="👨‍🎓 SELECT STUDENT", font=("Segoe UI", 12, "bold"),
        ...
        # Roll No Selection
        Label(form_frame, text="Roll No.", font=FONT_TITLE,
              bg=CARD_BG, fg=SUBTEXT).grid(row=0, column=0, padx=10, pady=(10, 5), sticky=W)
        self.cmb_roll = ttk.Combobox(form_frame, textvariable=self.var_roll, 
                                     values=self.roll_list, font=FONT_BODY,
                                     state="readonly", width=32)
        self.cmb_roll.grid(row=1, column=0, padx=10, pady=(0, 10), sticky=EW)
        ...
        btn_search.grid(row=2, column=0, padx=10, pady=5, sticky=EW)
        btn_search.bind("<Return>", lambda e: self._search())
        
        # Name - Fixed with better visibility
        Label(form_frame, text="Name", font=FONT_TITLE,
              bg=CARD_BG, fg=SUBTEXT).grid(row=3, column=0, padx=10, pady=(15, 5), sticky=W)
        self.txt_name = Entry(form_frame, textvariable=self.var_name, font=FONT_BODY,
                              bg=LIGHT_BG, fg=WHITE, relief=SOLID, bd=1, width=35,
                              state="readonly", readonlybackground=LIGHT_BG, 
                              disabledforeground=WHITE)
        self.txt_name.grid(row=4, column=0, padx=10, pady=(0, 10), sticky=EW)
        
        # Course - Fixed with better visibility
        Label(form_frame, text="Course", font=FONT_TITLE,
              bg=CARD_BG, fg=SUBTEXT).grid(row=5, column=0, padx=10, pady=(5, 5), sticky=W)
        self.txt_course = Entry(form_frame, textvariable=self.var_course, font=FONT_BODY,
                                bg=LIGHT_BG, fg=WHITE, relief=SOLID, bd=1, width=35,
                                state="readonly", readonlybackground=LIGHT_BG,
                                disabledforeground=WHITE)
        self.txt_course.grid(row=6, column=0, padx=10, pady=(0, 15), sticky=EW)
        
        # Separator
        Frame(form_frame, bg=SEP, height=2).grid(row=7, column=0, padx=10, pady=10, sticky=EW)
        
        # Subject Entry Section
        Label(form_frame, text="➕ ADD SUBJECT", font=FONT_TITLE,
              bg=CARD_BG, fg=SUBTEXT).grid(row=8, column=0, padx=10, pady=(5, 5), sticky=W)
        
        # Subject Name
        Label(form_frame, text="Subject Name", font=FONT_TITLE,
              bg=CARD_BG, fg=SUBTEXT).grid(row=9, column=0, padx=10, pady=(5, 5), sticky=W)
        self.txt_subject = Entry(form_frame, textvariable=self.var_subject, font=FONT_BODY,
                                 bg=LIGHT_BG, fg=WHITE, relief=SOLID, bd=1, width=35,
                                 insertbackground=WHITE)
        self.txt_subject.grid(row=10, column=0, padx=10, pady=(0, 10), sticky=EW)
        self.txt_subject.bind("<Return>", self._on_enter_pressed)
        self.entry_fields.append(self.txt_subject)
        
        # Marks Obtained
        Label(form_frame, text="Marks Obtained", font=FONT_TITLE,
              bg=CARD_BG, fg=SUBTEXT).grid(row=11, column=0, padx=10, pady=(5, 5), sticky=W)
        self.txt_obtained = Entry(form_frame, textvariable=self.var_obtained, font=FONT_BODY,
                                  bg=LIGHT_BG, fg=WHITE, relief=SOLID, bd=1, width=35,
                                  insertbackground=WHITE)
        self.txt_obtained.grid(row=12, column=0, padx=10, pady=(0, 10), sticky=EW)
        self.txt_obtained.bind("<Return>", self._on_enter_pressed)
        self.entry_fields.append(self.txt_obtained)
        
        # Full Marks
        Label(form_frame, text="Full Marks", font=FONT_TITLE,
              bg=CARD_BG, fg=SUBTEXT).grid(row=13, column=0, padx=10, pady=(5, 5), sticky=W)
        self.txt_full = Entry(form_frame, textvariable=self.var_full, font=FONT_BODY,
                              bg=LIGHT_BG, fg=WHITE, relief=SOLID, bd=1, width=35,
                              insertbackground=WHITE)
        self.txt_full.grid(row=14, column=0, padx=10, pady=(0, 10), sticky=EW)
        self.txt_full.bind("<Return>", self._on_enter_pressed)
        self.entry_fields.append(self.txt_full)
        
        # Add Subject Button
        btn_add = Button(form_frame, text="➕ Add Subject", font=FONT_TITLE,
                        bg=GREEN, fg=WHITE, cursor="hand2", relief=FLAT,
                        command=self._add_subject, pady=6)
        btn_add.grid(row=15, column=0, padx=10, pady=(10, 5), sticky=EW)
        btn_add.bind("<Return>", lambda e: self._add_subject())
        
        # Action Buttons
        btn_frame = Frame(form_frame, bg=CARD_BG)
        btn_frame.grid(row=16, column=0, padx=10, pady=(15, 0), sticky=EW)
        
        btn_frame.grid_columnconfigure(0, weight=1)
        btn_frame.grid_columnconfigure(1, weight=1)
        btn_frame.grid_columnconfigure(2, weight=1)
        
        btn_save = Button(btn_frame, text="💾 Save All", font=FONT_TITLE,
                         bg=GREEN, fg=WHITE, cursor="hand2", relief=FLAT,
                         command=self._save_all, pady=8)
        btn_save.grid(row=0, column=0, padx=5, sticky=EW)
        btn_save.bind("<Return>", lambda e: self._save_all())
        
        btn_remove = Button(btn_frame, text="🗑 Remove Row", font=FONT_TITLE,
                           bg="#C0392B", fg=WHITE, cursor="hand2", relief=FLAT,
                           command=self._remove_row, pady=8)
        ...
        # Table Frame
        table_frame = LabelFrame(right_panel, text=" Subjects & Marks ", font=FONT_TITLE,
        ...
        # Summary bar
        self.lbl_summary = Label(right_panel,
        ...
        # PDF button
        btn_pdf = Button(right_panel, text="📄 Generate PDF Result Card",
                        font=FONT_TITLE, bg=ORANGE, fg=WHITE,
                        cursor="hand2", relief=FLAT, activebackground="#C0541F",
                        command=self._generate_pdf, pady=10)
        btn_pdf.pack(fill=X)
    ...
    def _on_enter_pressed(self, event):
        ...
        try:
            ...
            if index < len(self.entry_fields) - 1:
                ...
                # If last field, trigger add subject
                self._add_subject()
        except ValueError:
            pass
        return "break"
    ...
    def _refresh_tree(self):
        ...
        if total_full > 0:
            ...
            self.lbl_summary.config(
                ...
                bg=GREEN if overall >= 40 else "#C0392B"
            )
        else:
            self.lbl_summary.config(
                text="Total: 0 / 0   |   Percentage: 0.00%   |   Grade: —   |   Result: —",
